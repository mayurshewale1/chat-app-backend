...
exports.uploadChatVoice = async (req, res) => {
  const { chatId } = req.params;
  if (!req.file) return res.status(400).json({ message: 'No voice file provided' });
  ...
  try {
    ...
    const voicePath = `/uploads/voice-notes/${req.file.filename}`;
    return res.status(201).json({ path: voicePath });
  } catch (err) {
    console.error(err);
    return res.status(500).json({ message: 'Server error' });
  }
